...
**The focus border** is a derived surface applied by `BorderSurfaceApplier`. `BorderWindow` drives one persistent `BorderLayerPanel`: a transparent, nonactivating `NSPanel` with a `CAShapeLayer` and disabled implicit animations. It orders the panel below the exact target through SkyLight. Target-level metadata is queried asynchronously and accepted only when both PID and WID match; until then, the border uses that same target's cached level or level 0. A failed lookup permits one later border-only retry. Translation-only updates reuse the layer path and cached metadata.

The target frame is rounded once to physical pixels and the panel's border geometry expands outward by the configured width. The layer uses an even-odd fill with a transparent rounded target cutout and an outer radius equal to the sampled target radius plus the border width, so no border pixel intentionally enters the target silhouette. Corner sampling is deferred until the workspace has no viewport motion and the target has no pending AX frame write; animation ticks reuse cached or default radii. `targetFrameOnScreen` records the rounded target while `frameOnScreen` reports the expanded surface. The surface remains registered with `SurfaceCoordinator` by window number and opts out of the screenshot window picker through `IgnoreForScreencaptureWindowSelection`; that property is invisible to full-screen captures and screen recording.

Border appearance is separate from layout geometry. Existing solid borders remain the default when `[borders.gradient]` and `[borders.glow]` are absent. The optional gradient supports two local-surface directions and is clipped to the existing even-odd annulus. Glow is rendered first as a dedicated color layer below the border: it reuses the border's solid color or gradient endpoints and is masked by concentric stroked bands that fade the color out to zero across the overlay padding, then the solid or gradient annulus is rendered over it. An enabled glow expands only the private overlay surface; layout clearance, gaps, target frames, and resize hit-testing remain width-based. `MouseEventHandler` uses the ring frame rather than the glow-expanded surface for resize interaction. Gradient glow follows the border gradient spatially because the glow color layer reuses the border gradient's endpoints and unit points. JankyBorders features intentionally not adopted here include centered-stroke/`width + 8` geometry, arbitrary gradient angles, and spatially varying gradient blur; OmniWM keeps its exterior annulus and width-based layout contract.

The settings UI exposes gradient enablement, direction, start/end colors, glow enablement, radius, and opacity. These values are optional nested TOML tables and are applied transactionally: absent tables preserve legacy solid behavior, malformed or out-of-range values preserve the previous valid appearance, and style-only changes schedule border-only reconciliation. The existing animation policy remains intact: display-link ticks may use cached geometry for continuity, while terminal reconciliation returns to the complete/live derivation path.
