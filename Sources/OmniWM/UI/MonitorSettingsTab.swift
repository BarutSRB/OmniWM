...
struct MonitorSettingsTab: View {
    ...
    var body: some View {
        SettingsPage(
            ...
            Section("Monitor Roles") {
                ForEach(Array(settings.monitors.ranking.enumerated()), id: \.element) { index, entry in
                    RankedMonitorRow(
                        ...
                        canMoveUp: index > 0,
                        canMoveDown: index < settings.monitors.ranking.count - 1,
                        moveUp: {
                            setMonitorRanking(MonitorRanking.moving(settings.monitors.ranking, from: index, by: -1))
                        },
                        moveDown: {
                            setMonitorRanking(MonitorRanking.moving(settings.monitors.ranking, from: index, by: 1))
                        },
                        remove: {
                            setMonitorRanking(MonitorRanking.removing(settings.monitors.ranking, at: index))
                        }
                    )
                }
                ...
                Menu("Add Monitor") {
                    ForEach(addableRankingEntries, id: \.self) { entry in
                        Button(rankedMonitorLabel(for: entry)) {
                            setMonitorRanking(settings.monitors.ranking + [entry])
                        }
                    }
                }
    ...
    private func rankedMonitorRoleLabel(at index: Int) -> String {
        let ranks = MonitorRanking.effectiveRanks(ranking: settings.monitors.ranking, monitors: connectedMonitors)
    ...
    private var addableRankingEntries: [OutputId] {
        MonitorRanking.addable(connected: connectedMonitors, ranking: settings.monitors.ranking)
    }
